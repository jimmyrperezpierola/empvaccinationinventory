...
const validationSchema = Yup.object({
    ...
    mobilePhoneNumber: Yup.string("Enter a Mobile Phone Number").max(20, "mobilePhoneNumber must contain maximun 20 characters").matches(/^[0-9 ]+$/, "Must be only numbers"),
    vaccineTimes: Yup.string("Enter a Vaccine Times").max(2, "vaccineTimes must contain maximun 2 numbers").matches(/^[0-2 ]+$/, "Must be only numbers"),
  });

const UpdateMyInfo = (props) => {
  ...
  const getData = async () => {
    ...
    values.birthDate = employee.data.birthDate;
    values.homeAddress = employee.data.homeAddress;
    values.mobilePhoneNumber = employee.data.mobilePhoneNumber;
    values.isVaccinated = employee.data.isVaccinated;
    values.vaccineName = employee.data.vaccineName;
    values.vaccineDate = employee.data.vaccineDate;
    values.vaccineTimes = employee.data.vaccineTimes;
    ...
    return (
        ...
        <div className={styles.container}>
          <Paper elevation={1} className={styles.paper}>
            ...
            <br />
            <Formik
              render={props => <UpdateMyInfoForm {...props} />}
...
export default withStyles(styles)(UpdateMyInfo); 